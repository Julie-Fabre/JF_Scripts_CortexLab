...
for iInjection = 1:size(injectionAreas, 2)
    ...
    for iView = 1:3
        ...
        if collapseHemispheres 
            max_voxel_x = [projData.max_voxel_x]; 
            x_rel = max_voxel_x - ap_max/2;
            max_voxel_x(x_rel > 0) = ap_max/2 - x_rel(x_rel > 0); 
            
            max_voxel_y = [projData.max_voxel_y]; 
            y_rel = max_voxel_y - ml_max/2;
            max_voxel_y(y_rel > 0) = ap_max/2 - y_rel(y_rel > 0); 
        else
            max_voxel_x = [projData.max_voxel_x]; 
            max_voxel_y = [projData.max_voxel_y]; 
        end
        ...
        scatter3(max_voxel_x(theseTargets)/10, [projData(theseTargets).max_voxel_z]/10, max_voxel_y(theseTargets)/10, ...

...
figure();
plot(surround_t(1:end-1), motion_energy(:,iCondition)); hold on;
plotshaded(surround_t(1:end-1), [motion_energy(:,iCondition)' - motion_energy_std(:,iCondition)'; ...
motion_energy(:,iCondition)' + motion_energy_std(:,iCondition)'], 'b');
ymax = ylim;
line([0, 0],[ymax(1), ymax(2)], 'Color', 'red')
xlabel('time from wheel movement onset')
ylabel('motion energy (whole image)')
legend(['# trials = ' num2str(n_trials(iCondition))])
makepretty;

function quiescence_reset_t = JF_extrap_stimWheel_quiescence(n_trials, signals_events, t, ...
    block2timeline, timeline2block, block, quiescThreshold)
...
for curr_trial = 2:n_trials
    ...
    if isfield(signals_events, 'trialQuiescenceValues')
        curr_quiescence = signals_events.trialQuiescenceValues(curr_trial);
        curr_iti = signals_events.trialITIValues(curr_trial-1);
    else % not saved initially, take smallest value 
        curr_quiescence = block.paramsValues.quiescMin;
        curr_iti = block.paramsValues.itiMin;
    end

...
AP_cellraster({stimOn_times,wheel_move_time,signals_events.responseTimes(n_trials)', ...
    [signals_events.responseTimes(n_trials)'; unexpected_reward_times']}, ...
    {trial_conditions(:,1),trial_conditions(:,2), ...
    trial_conditions(:,3), [trial_conditions(:,5); 2*ones(size(unexpected_reward_times,2),1)]});

experiment = 2;
JF_load_experiment;
curr_shank=NaN;
theseImages = [4,6,12];
theseImages_trials = ismember(stimIDs, theseImages);
AP_cellrasterJF({stimOn_times(theseImages_trials), stimOn_times(theseImages_trials), stimOn_times(theseImages_trials)}, ...
    {trial_conditions(theseImages_trials,1), trial_conditions(theseImages_trials,2),...

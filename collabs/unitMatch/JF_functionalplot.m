...
% 
% 
% subplot(length(examples)+5, last_thisDate+2, 2*(last_thisDate+2)+1:last_thisDate+2*(last_thisDate+2)-1);
% hold on;
% %yyaxis left;
% %ll=lines(2);
% plot(1:last_thisDate, [NaN; NaN; bhvData.rewardRate(1:last_thisDate-2)])
% ylabel('reward rate (reward/s)');
% xlim([1.5, last_thisDate + 0.5])
% 
% subplot(length(examples)+5, last_thisDate+2, 3*(last_thisDate+2)+1:last_thisDate+3*(last_thisDate+2));
% hold on;
% 
% plot(1:last_thisDate, [NaN; NaN; bhvData.rewardAmount(1:last_thisDate-2)])
% 
% ylabel('frac. correct trials');
% 
% %yl.Color = ll(2,:);
% xlim([1.5, last_thisDate + 0.5])
% 
% %[hBins, hbIdx] = hist(bhvData.rxn(1:last_thisDate-2));

%% activity over population

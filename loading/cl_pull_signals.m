 expDef = strrep(block.expDef, '\', '/'); % for windows to UNIX paths
    ...
    switch expDef
        ...
        case {'vanillaChoiceworld', 'vanillaChoiceworldBias', 'vanillaChoiceworldNoRepeats'}
            ...
            case {'noGo_path2stage3_probaReward', 'noGo_path2stage3_4stim_noProba'} % stimType,
            % Hit/miss recorded for last trial, circshift to align
...
%            cl_correct_passive_photodiode; 
        ...
        case{ 'JF_choiceworldStimuli_wheel_left_center_right_mult'}
              stimOn_times = photodiode_flip_times(2:2:end);
            cl_correct_passive_photodiode; 
            ...
            trial_azimuths(signals_events.stim_idValues <= 16) = 0;
            trial_azimuths(signals_events.stim_idValues > 16 & signals_events.stim_idValues <= 32) = -90;
            trial_azimuths(signals_events.stim_idValues > 32) = 90;
           
            trial_conditions = ... ,
                [ceil(signals_events.stim_idValues)', trial_azimuths];
            trial_id = trial_conditions(:, 2);
            trial_conditions(trial_conditions(:, 1) > 16, 1) = trial_conditions(trial_conditions(:, 1) > 16, 1) - 16;
            trial_conditions(trial_conditions(:, 1) > 16, 1) = trial_conditions(trial_conditions(:, 1) > 16, 1) - 16;
            trial_conditions(trial_conditions(:, 1) == 14, 1)= 4;% stim 1
            trial_conditions(trial_conditions(:, 1) == 15, 1) = 6;% stim 3
            trial_conditions(trial_conditions(:, 1) == 16, 1) = 12;% stim 2
            stimIDs = trial_conditions(:, 1);

...
[timeline_filename, timeline_exists] = cl_cortexlab_filename(animal, thisDate, experiment, 'timeline');
if ~timeline_exists
    error([animal, ' ', thisDate, ' ', num2str(experiment), ': no timeline']);
end
...
if timeline_exists
    ...
    [wheel_velocity, wheel_move] = cl_parse_wheel(wheel_position, Timeline.hw.daqSampleRate);
...
% Load the block
[block_filename, block_exists] = cl_cortexlab_filename(animal, thisDate, experiment, 'block');
...
if exist('Timeline', 'var') && load_parts.cam
    ...
    % EYECAM
    [eyecam_dir, eyecam_exists] = cl_cortexlab_filename(animal, thisDate, experiment, 'eyecam');
    bad_eyecam = false;
    ...
    if eyecam_exists
        ...
        [eyecam_processed_filename, eyecam_processed_exists] = cl_cortexlab_filename(animal, thisDate, experiment, 'eyecam_processed');
        if eyecam_processed_exists
            eyecam = load(eyecam_processed_filename);
        end

        % Get camera times
        eyecam_fn = cl_cortexlab_filename(animal, thisDate, experiment, 'eyecam');
        ...
        if exist(eyecam_fn, 'file') && ~exist(eyecam_t_savefile, 'file')
            ...
            [eyecam_sync_frames, n_eyecam_frames] = cl_get_cam_sync_frames(eyecam_fn);
    ...
    % FACECAM
    [facecam_dir, facecam_exists] = cl_cortexlab_filename(animal, thisDate, experiment, 'facecam');
    bad_facecam = false;
    if facecam_exists
        if verbose
        ...
        % Get camera times
        facecam_fn = cl_cortexlab_filename(animal, thisDate, experiment, 'facecam');
        ...
        if exist(facecam_fn, 'file') && ~exist(facecam_t_savefile, 'file')
            ...
            % Get sync times for cameras (or load if already done)
            [facecam_sync_frames, n_facecam_frames] = cl_get_cam_sync_frames(facecam_fn);
        ...
        elseif exist(facecam_fn, 'file') && exist(facecam_t_savefile, 'file')
            ...
            facecam_exists = false;
        end

        % (old/unused: etGUI and facemap)
        [facecam_processed_filename, facecam_processed_exists] = cl_cortexlab_filename(animal, thisDate, experiment, 'facecam_processed');
        if facecam_processed_exists
            facecam = load(facecam_processed_filename);
        end

        % (output from AP_mouse_movie_movement)
        [facecam_movement_filename, facecam_movement_exists] = cl_cortexlab_filename(animal, thisDate, experiment, 'facecam_movement');
...
if load_parts.ephys
    ...
    if ~exist('kilosort_version', 'var') || kilosort_version == 2
        [ephys_path, ephys_exists] = cl_cortexlab_filename(animal, thisDate, experiment, 'ephys', site, recording);
        [ephysAP_path, ~] = cl_cortexlab_filename(animal, thisDate, experiment, 'ephys_includingCompressed', site, recording);
        isSpikeGlx = contains(ephysAP_path, '_g');
    elseif exist('kilosort_version', 'var') && kilosort_version == 1
        [ephys_path, ephys_exists] = cl_cortexlab_filename(animal, thisDate, experiment, 'ephys_ks1', site, recording);
    end
end
if load_parts.ephys
    if ephys_exists && load_parts.ephys
        ...
        protocols_list = cl_list_experiments(animal, thisDate);
        ...
        if load_sync
            try
                [spike_times_timeline, bad_flipper] = cl_align_ephys_to_timeline(animal, thisDate, isSpikeGlx, flipper_flip_times_timeline, ...
        ...
        if exist('locationKeep', 'var') && ~isempty(locationKeep)
            ...
            if isnumeric(locationKeep)
                ...
                probe2ephysFile = cl_cortexlab_filename(animal, [], [], 'probe2ephys', [], []);
                load(probe2ephysFile)
                histoFile = cl_cortexlab_filename(animal, [], [], 'histo', [], []);
                load(histoFile)

                if isSpikeGlx
                    probeNumber = find([probe2ephys.thisDate] == curr_thisDate & [probe2ephys.site] == site & [probe2ephys.thisDate] == shank);
                else
                    probeNumber = find([probe2ephys.thisDate] == curr_thisDate & [probe2ephys.site] == site);
            ...
            %             probeccf = cl_cortexlab_filename(animal, [], [], 'histo', [], []);
            %             load(probeccf)
        ...
        elseif exist('unitType', 'var') && loadClusters
            ...
            if exist('locationKeep', 'var') && ~isempty(locationKeep)
                if isnumeric(locationKeep)
                    ...
                    if ~isnan(shank)
                        probeNumber = find([probe2ephys.thisDate] == curr_thisDate & [probe2ephys.thisDate] == site & [probe2ephys.thisDate] == shank);
                    else
                        probeNumber = find([probe2ephys.thisDate] == curr_thisDate & [probe2ephys.thisDate] == site);
                ...
                template_exists = ismember(0:max(spike_templates_0idx), unique(spike_templates_0idx));
                theseTemplates = template_depths(template_exists) >= min(theseDepths) & template_depths(template_exists) <= max(theseDepths); %correct depth units
                good_templates = unitType' == 1 & theseTemplates';
                good_templates_idx = find(good_templates) - 1;
            ...
            warning([animal, ' ', thisDate, ' - no cluster groups']);
            if verbose
...
if load_parts.ephys
    if ephys_exists && load_parts.ephys && exist('lfp_channel', 'var')
        ...
        if ~isSpikeGlx
            ...
            [data_path, data_path_exists] = cl_cortexlab_filename(animal, thisDate, experiment, 'ephys_dir', site);
            ...
            [filename, file_exists] = cl_cortexlab_filename(animal, thisDate, experiment, 'mainfolder', site, recording);

...
for iAnimal = 1:size(animalsAll, 2)
    ...
    protocol = 'noGo_stage'; %'location'; %protocol name contains this name
    ...
    for curr_day = 1:length(experiments)
        ...
        for curr_day_exps = 1:length(experiment_num)
            try
                experiment = experiment_num(curr_day_exps);

                [block_filename, block_exists] = AP_cortexlab_filenameJF(animal, day, experiment, 'block');
                if ~isempty(block_filename)
                    load(block_filename)

                    trNum(curr_day_exps) = length(block.events.newTrialTimes);
                else
                    trNum(curr_day_exps) = 0;
                end
            catch
            end
        end
        ...
        for curr_experiment = find(trNum == max(trNum))
            try
                ...
                if contains(block.expDef, 'noGo_stage4') || contains(block.expDef, 'noGo_stage5')
                ...
                trialTypes = {go1Trials, go2Trials, noGoTrials};
                if isfield(block.events, 'repeatTrialValues')
                    repeatOnMisses = block.events.repeatTrialValues(response_trials) > 0;
                else
                ...
                for iStimType = 1:3
                    correctTrials(iStimType) = sum(block.events.hitValues(response_trials) == 1 & trialTypes{iStimType});

                    nTrials(iStimType) = sum(~repeatOnMisses(response_trials) & trialTypes{iStimType});

                    goLeft(iStimType) = sum(~repeatOnMisses(response_trials) & ...
                        block.events.hitValues(response_trials) == 1 & trialTypes{iStimType});

                    noGo(iStimType) = sum(~repeatOnMisses(response_trials) & ...
                        block.events.hitValues(response_trials) == 0 & trialTypes{iStimType});
                end

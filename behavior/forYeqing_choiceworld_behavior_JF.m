        ...
        for curr_experiment = length(experiment_num)
            ...
            elseif isfield(block.events, 'orientationValues')%different orientations
                ...
                for iCond = 1:size(bhv.conditions, 1)
                    ...
                    bhv.go_left_trialsNoGo(curr_day, iCond) = numel(find(trial_stim == bhv.conditions(iCond) & ~repeatOnMisses & block.events.orientationValues(1:end-1) ==45 &block.events.responseValues == -1)); %&~repeatOnMisses

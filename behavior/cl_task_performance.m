%clear all;
%clc;
function bhvOut = cl_task_performance(animalsAll)
plotAll = false;
...
for iAnimal = 1:size(animalsAll, 2)
    ...
    experiments1 = cl_find_experiments(animal, protocol, flexible_name);
    experiments2 = cl_find_experiments(animal, protocol2, flexible_name);
    experiments3 = cl_find_experiments(animal, protocol3, flexible_name);
...
%                 wheel_position_deg, pull_times);
    ...
    day_num = (cellfun(@(x) datenum(x), {experiments(keep_day).thisDate}));
    day_num_label = datestr(day_num, 'mm-dd');
    day_labels_temp = cellfun(@(day, protocol) [day(6:end)], ...
        {experiments(keep_day).thisDate},bhv.expDefName(keep_day), 'uni', false);
...
%    bhvOut(iAnimal).protocol_idx = protocol_idx;
 %   bhvOut(iAnimal).thisDate_labels = day_labels;

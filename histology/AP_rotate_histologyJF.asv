function AP_rotate_histologyJF(im_path)
...
for curr_im = 1:length(slice_im)
    imshow(slice_im_pad(:,:,:,curr_im), [], 'Colormap', gray);
...
for curr_im = 1:length(slice_im)
    ...
    im_aligned(:,:,:,curr_im) = ...
        ...
        [x_diff,y_diff]),real(angle_diff),'nearest','crop');
...
gui_data.histology_im_h = imagesc(gui_data.im_aligned(:,:,:,1), ...

...
tv_coronal_20um = tv_coronal(:,:,1:10:end);
...
p.MaximumNumberOfIterations=900;
...
reg=elastix(squeeze(RGBimg(:,:,:,1)),tv_coronal_20um ,[],'elastix_default.yml','paramstruct',p);
%save as multipage tiff 

saveastiff(reg, ['D:/JF025/reg',p.Transform, '-',num2str(p.SP_alpha), '-',num2str(p.NumberOfSpatialSamples),'-',num2str(p.MaximumNumberOfIterations),'.tiff']); 

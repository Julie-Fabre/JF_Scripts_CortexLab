...
implantDay = {'2021-08-27', '2022-01-24', '2021-05-20', '2021-07-20'};
plotDriftMap = 0;
plotActivity = 1;
ex = [];
...
for iAnimal = thisAnimal %1:size(animalsAll, 2)
    ...
    implantDay = recordingInfoTable.implantDay{1};
    sites = str2double(recordingInfoTable.nSites{1});
    startTrainingDayNum = find(arrayfun(@(x) recordingInfoTable.Phase{x} == '1', 1:size(recordingInfoTable.Phase, 1)), 1, 'first');
    startStage4DayNum = find(arrayfun(@(x) recordingInfoTable.Phase{x} == '4', 1:size(recordingInfoTable.Phase, 1)), 1, 'first');

    figure(4);
    clf;
    animal = animalsAll{1, iAnimal}; %this animal
    %implantDay = implantDay{1, iAnimal}; %this animal
    ...
    startTraining = datetime(str2double(startTraining{1}), str2double(startTraining{2}), str2double(startTraining{3}));

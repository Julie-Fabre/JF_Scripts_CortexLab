...
%% info
animalsAll = {'JF051', 'JF067', 'JF078', 'JF082', 'JF084'};
% DMS, DMS, DMS + SNr, 
...
for iAnimal = thisAnimal %1:size(animalsAll, 2)
    ...
    implantSide = recordingInfoTable.implantSide{1};
      implantSide = str2num(implantSide);
...
%                     makeprettyLarge;
                    ...
                    figure(4);
                    title(num2str(curr_day)); hold on;
                    if curr_day >= startTrainingDayNum
                        subplot(5, size(experiments, 1), curr_day)
                        if curr_day >= startStage4DayNum
                            hold on;
                            scatter([1, 2, 3], bhvData.goLeft(currDayIdx-startTrainingDayNum+1, [1, 3, 2])./ ...
                                bhvData.nTrials(currDayIdx-startTrainingDayNum+1, [1, 3, 2]), [], rgb('DarkBlue'), 'filled')
                            p1 = plot([1, 2, 3], bhvData.goLeft(currDayIdx-startTrainingDayNum+1, [1, 3, 2])./ ...
                                bhvData.nTrials(currDayIdx-startTrainingDayNum+1, [1, 3, 2]), 'Color', rgb('DarkBlue'));
                            makepretty;
                            scatter([1, 2, 3], bhvData.noGo(currDayIdx-startTrainingDayNum+1, [1, 3, 2])./ ...
                                bhvData.nTrials(currDayIdx-startTrainingDayNum+1, [1, 3, 2]), [], rgb('Red'), 'filled')
                            p2 = plot([1, 2, 3], bhvData.noGo((currDayIdx - startTrainingDayNum + 1), [1, 3, 2])./ ...
                                bhvData.nTrials(currDayIdx-startTrainingDayNum+1, [1, 3, 2]), 'Color', rgb('Red'));
                            makepretty;
                            if curr_day == startStage4DayNum
                            ...
                            plot(bhvData.binBorders(1:end-1), movingFrac(currDayIdx-startTrainingDayNum+1, :), 'b');
                            box off;
                            if curr_day == startTrainingDayNum
                                ylabel('frac. mov.')
                    ...
                    if ~any(sum(ismember(trial_conditions, [4, 0]),2)==2) || ~any(sum(ismember(trial_conditions, [4, -90]),2)==2)
                        disp('passive error')
                    end
                    try
                        stimType(trial_conditions(no_move_trials, 1) == 4 & trial_conditions(no_move_trials, 2) == -implantSide) = 1; % go1 stim, contra
                        go1Contra = 1;
                    catch
                        go1Contra = 0;
                    end
                    ...
                    try
                        stimType(trial_conditions(no_move_trials, 1) == 7 & trial_conditions(no_move_trials, 2) == -implantSide) = 3; % no go stim, contra
                        noGoContra = 1;
                    catch
                        noGoContra = 0;
                    end
                    ...
                    try
                        stimType(ismember(trial_conditions(no_move_trials, 1), 6) & trial_conditions(no_move_trials, 2) == -implantSide) = 5; % other stims, contra
                        go2Contra = 1;
                    catch
                        go2Contra = 0;
                    end
                    try
                        stimType(ismember(trial_conditions(no_move_trials, 1), 6) & trial_conditions(no_move_trials, 2) == 0) = 6; % % other stims, center
                        go2Center = 1;
                    catch
                        go2Center = 0;
                    end
                    ...
                    subplot(5, size(experiments, 1), curr_day+(size(experiments, 1)*(iLocation+1)))
                    ...
                    spikes1 = spike_depths >= depths{iAnimal}{:}(1) & spike_depths <= depths{iAnimal}{:}(2);
                    [~, ~, ~, ~, ~, ~, ~, pltTime4, pltY4, ~] = ...
                    ...
                    savePSTH_noNorm(:, curr_day, 1, 1) = pltY4(1 , :);
                    savePSTH_noNorm(:, curr_day, 2, 1) = pltY4(3 , :);
                    savePSTH_noNorm(:, curr_day, 3, 1) = pltY4(5 , :);
                
                    savePSTH_noNorm(:, curr_day, 1, 2) = pltY4(2, :);
                    savePSTH_noNorm(:, curr_day, 2, 2) = pltY4(4, :);
                    savePSTH_noNorm(:, curr_day, 3, 2) = pltY4(6, :);

                    pltY4_norm = (pltY4 - nanmean(pltY4(:, pltTime4 < 0), 2)) ./ nanmean(pltY4(:, pltTime4 < 0), 2);

                    ll = [1, 1, 2, 2, 3, 3];
                    ll = ll(find(logical(mod(uniquePoss, 2)))+iLocation);
                    %ll = ll(find(uniquePoss));
                    %pp = pltY4(ll, :);
                    pp = pltY4_norm(find(logical(mod(uniquePoss, 2)))+iLocation, :);
...
%figure();plot(pltTime4,squeeze(pp(1,:)))
                    savePSTH(:, curr_day, 1,  1) = pltY4_norm(1 , :);
                    savePSTH(:, curr_day, 2,  1) = pltY4_norm(3 , :);
                    savePSTH(:, curr_day, 3,  1) = pltY4_norm(5 , :);
                
                    savePSTH(:, curr_day, 1,  2) = pltY4_norm(2, :);
                    savePSTH(:, curr_day, 2,  2) = pltY4_norm(4, :);
                    savePSTH(:, curr_day, 3,  2) = pltY4_norm(6, :);
                

                    xlim([pltTime4(1), 0.25])
                    line([0, 0], [-0.37, 1], 'Color', 'k')
                    box off;

                    if curr_day == 1 && iLocation ==0
...
%     figure();
%     clf;
%     allColors = colormap(brewermap(size(savePSTH,2),'Spectral'));
%     for iStim = 1:3
%         subplot(2,3,iStim)
%         for iDay = 1:size(savePSTH,2)
%             plot(squeeze(savePSTH(1,iDay,iStim,:)), 'Color', allColors(iDay,:)); hold on;
%             makepretty;
%         end
%     end
% 
...
%     for iStim = 1:3
%         subplot(2,3,iStim)
%         imagesc(squeeze(savePSTH_noNorm(1,:,iStim,:))); hold on;
%         makepretty;
% 
%         subplot(2,3,iStim+3)
%         imagesc(squeeze(savePSTH_noNorm(2,:,iStim,:))); hold on;
%         makepretty;
%         
%     end
    ...
    for iStim = 1:3
        subplot(2,3,iStim)
        imagesc(squeeze(savePSTH(:,:,iStim,1))'); hold on;
        makepretty;
        %clim([-0.2, 0.3]);

        subplot(2,3,iStim+3)
        imagesc(squeeze(savePSTH(:,:,iStim,2))'); hold on;
        makepretty;
        %clim([-0.2, 0.3]);
    ...
    figure();
averageIncGo1Center = nanmean(savePSTH(1, :, 1, 301:451), 4);
averageIncGo2Center = nanmean(savePSTH(1, :, 2, 301:451), 4);
averageIncNoGoCenter = nanmean(savePSTH(1, :, 3, 301:451), 4);

averageIncGo190 = nanmean(savePSTH(2, :, 1, 301:451), 4);
averageIncGo290 = nanmean(savePSTH(2, :, 2, 301:451), 4);
averageIncNoGo90 = nanmean(savePSTH(2, :, 3, 301:451), 4);



scatter(1:size(averageIncGo1Center,2), movmean(averageIncGo1Center, 2), 'filled');
hold on;
scatter(1:size(averageIncGo2Center,2), movmean(averageIncGo2Center, 2), 'filled');
scatter(1:size(averageIncNoGoCenter,2), movmean(averageIncNoGoCenter, 2), 'filled');

scatter(1:size(averageIncGo190,2), movmean(averageIncGo190, 2), 'filled');
scatter(1:size(averageIncGo290,2), movmean(averageIncGo290, 2), 'filled');
scatter(1:size(averageIncNoGo90,2), movmean(averageIncNoGo90, 2), 'filled');

end

%%

% average go 1 center, periph before , critical time, after 
% average no go center, periph before , critical time, after 
% average go2 center, periph before , critical time, after 

figure();
subplot(231)
plot(pltTime4,squeeze(nanmean(savePSTH(:, 1:2, 1,1),2))); hold on; 
plot(pltTime4,squeeze(nanmean(savePSTH(:, 3:4, 1,1),2)));
plot(pltTime4,squeeze(nanmean(savePSTH(:, 5:6, 1,1),2)));
xlabel('time from stimulus onset (s)')
ylabel('dFR/FR')
legend('before learning go 1', 'post learning go 1', 'post learning no go')

makepretty;

subplot(232)
plot(pltTime4,squeeze(nanmean(savePSTH(:, 1:2, 3,1),2))); hold on; 
plot(pltTime4,squeeze(nanmean(savePSTH(:, 3:4, 3,1),2)));
plot(pltTime4,squeeze(nanmean(savePSTH(:, [8:9, 14], 3,1),2)));
makepretty;

subplot(233)
plot(pltTime4,squeeze(nanmean(savePSTH(:, 1:2, 2,1),2))); hold on; 
plot(pltTime4,squeeze(nanmean(savePSTH(:, 3:4, 2,1),2)));
plot(pltTime4,squeeze(nanmean(savePSTH(:, [8:9, 14], 2,1),2)));
makepretty;


subplot(234)
plot(pltTime4,squeeze(nanmean(savePSTH(:, 1:2, 1,2),2))); hold on; 
plot(pltTime4,squeeze(nanmean(savePSTH(:, 3:4, 1,2),2)));
plot(pltTime4,squeeze(nanmean(savePSTH(:, 5:6, 1,2),2)));
makepretty;

subplot(235)
plot(pltTime4,squeeze(nanmean(savePSTH(:, 1:2, 3,2),2))); hold on; 
plot(pltTime4,squeeze(nanmean(savePSTH(:, 3:4, 3,2),2)));
plot(pltTime4,squeeze(nanmean(savePSTH(:, [8:9, 14], 3,2),2)));
makepretty;

subplot(236)
plot(pltTime4,squeeze(nanmean(savePSTH(:, 1:2, 2,2),2))); hold on; 
plot(pltTime4,squeeze(nanmean(savePSTH(:, 3:4, 2,2),2)));
plot(pltTime4,squeeze(nanmean(savePSTH(:, [8:9, 14], 2,2),2)));
makepretty;
...
for iAnimal = 1%:size(animals, 2)
    ...
    for iDay = 1:2%:size(experimentsBhv, 1)
        try
            ...
            theseST = spike_templates(spike_depths > 1000 & spike_depths < 2000);
...
ylabel('trial (sorted by stim increase)')
subplot(122)
...
xlabel('time from move')
ylabel('trial (sorted by move increase)')
subplot(122)
im = (m2CellPSTHstim - nanmean(m2CellPSTHstim(:, 1:200), 2)) ./ nanmean(m2CellPSTHstim(:, 1:200), 2);
%[~, sorted_im_idx] = sort(nanmean(im(:,300:500),2));
colormap(brewermap([], '*RdBu'));

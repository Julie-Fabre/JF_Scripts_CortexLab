...
figure(100);
figure(101);
clearvars responsive_cell_region_up responsive_cell_region_down
for iRegion = 1:3
    ...
    elseif method == 1%method 1: high zscore
         these_units = passive_data.unit_area == iRegion & ...
        (passive_data.unitType' == 1 ) & abs(nanmean(zscore_psth(:, 260:300), 2)) > 0.25;
    elseif method ==  2
   ...
   responsive_cell_region_up(iRegion) = sum(these_units & nanmean(zscore_psth(:, 260:300), 2)>0 & keep_these)/...
       sum(passive_data.unit_area == iRegion & ...
        (passive_data.unitType' == 1) & keep_these) ;

   responsive_cell_region_down(iRegion) = sum(these_units & nanmean(zscore_psth(:, 260:300), 2)<0 & keep_these)/sum(passive_data.unit_area == iRegion & ...
        (passive_data.unitType' == 1 ) & keep_these) ;
    ...
    if sum(these_units) > 0
       ...
       % title(regions{iRegion})
        ...
        caxis([-2.5, 2.5])
        yy=ylim;
        line([0,0], [yy(1), yy(2)], 'Color', rgb('DarkRed'))

        figure(101);
         subplot(3,1,iRegion)
        b = barh(1:2, [responsive_cell_region_up(iRegion).*100, responsive_cell_region_down(iRegion).*100], 0.2, 'facecolor', 'flat');
...
for iRegion = 1%:3
    ...
    keep_these = sum(isnan(zscore_psth), 2) < 100 & sum(zscore_psth == 0, 2) < 100 &  abs(nanmean(zscore_psth(:, 220:250), 2)) < 0.25 & ...
      ...
      responsive_cell_region_up(iRegion,iSubReg) = sum(these_units & nanmean(zscore_psth(:, 260:300), 2)>0 & keep_these)/...
       sum(passive_data.unit_area == iRegion & ...
        (passive_data.unitType' == 1 | passive_data.unitType' == 2) & keep_these) ;

   responsive_cell_region_down(iRegion,iSubReg) = sum(these_units & nanmean(zscore_psth(:, 260:300), 2)<0 & keep_these)/sum(passive_data.unit_area == iRegion & ...
        (passive_data.unitType' == 1 | passive_data.unitType' == 2) & keep_these) ;
        ...
        if sum(these_units) > 0
            ...
            figure(103)
            subplot(1, size(unique_subReg,1), iSubReg)
            b = barh(1:2, [responsive_cell_region_up(iRegion,iSubReg), responsive_cell_region_down(iRegion,iSubReg)], 0.2, 'facecolor', 'flat');
b.CData = [regionColors{iRegion}; regionColors{iRegion}];
yticks([1:2])
yticklabels({'^', 'v'})
makepretty;
...
figure(105);
   if method == 0
    ...
    keep_these = sum(isnan(zscore_psth), 2) < 100 & sum(zscore_psth == 0, 2) < 100 & ...
     ...
     responsive_cell_region_up(iRegion,iSubReg,iUnitType) = sum(these_units & nanmean(zscore_psth(:, 260:300), 2)>0 & keep_these)/...
       sum(passive_data.unit_area == iRegion & ...
        (passive_data.unitType' == 1 | passive_data.unitType' == 2) & keep_these) ;

   responsive_cell_region_down(iRegion,iSubReg,iUnitType) = sum(these_units & nanmean(zscore_psth(:, 260:300), 2)<0 & keep_these)/sum(passive_data.unit_area == iRegion & ...
        (passive_data.unitType' == 1 | passive_data.unitType' == 2) & keep_these) ;
...
figure(105)
 subplot(size(unique_unit,1), size(unique_subReg,1), (iSubReg-1)*size(unique_unit,1)+iUnitType)
              b = barh(1:2, [responsive_cell_region_up(iRegion,iSubReg,iUnitType), responsive_cell_region_down(iRegion,iSubReg,iUnitType)], 0.2, 'facecolor', 'flat');
b.CData = [regionColors{iRegion}; regionColors{iRegion}];
yticks([1:2])
yticklabels({'^', 'v'})
makepretty;
...
prettify_plot;
    ...
    keep_these = sum(isnan(zscore_psth), 2) < 100 & sum(zscore_psth == 0, 2) < 100 & ...
     ...
     responsive_cell_region_up(iRegion,iUnitType) = sum(these_units & nanmean(zscore_psth(:, 260:300), 2)>0 & keep_these)/...
       sum(passive_data.unit_area == iRegion & ...
        (passive_data.unitType' == 1 ) & keep_these) ;

   responsive_cell_region_down(iRegion,iUnitType) = sum(these_units & nanmean(zscore_psth(:, 260:300), 2)<0 & keep_these)/sum(passive_data.unit_area == iRegion & ...
        (passive_data.unitType' == 1 ) & keep_these) ;
...
figure(currF)
            ...
            figure(106)
            subplot(3, size(unique_unit,1), (iRegion-1)*size(unique_unit,1)+iUnitType)
              b = barh(1:2, [responsive_cell_region_up(iRegion,iUnitType)*100, responsive_cell_region_down(iRegion,iUnitType)*100], 0.2, 'facecolor', 'flat');
b.CData = [regionColors{iRegion}; regionColors{iRegion}];
yticks([1:2])
yticklabels({'^', 'v'})
makepretty;

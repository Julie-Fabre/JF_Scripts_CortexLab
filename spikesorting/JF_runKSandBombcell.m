...
[ephysAPfile, ~] = cl_cortexlab_filename(animal, date, experiment, 'ephys_includingCompressed', site, recording);
...
elseif contains(ephysAPfile, '.cdat') %skip these for now
    runKS=0;
else
    rootZ = fileparts(ephysAPfile);
...
%% kilosort
if (isempty(kilosortExists) || rerunKS) && runKS
...
if isempty(syncExists)
    ...
    if isSpikeGlx
        [ephysKSfile, ~] = cl_cortexlab_filename(animal, date, experiment, 'ephys', site, recording);
...
bc_ephysPropertiesPipeline_JF(animal, date, site, recording, experiment, rerunEP, runEP, region);

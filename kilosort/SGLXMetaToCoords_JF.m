...
saveas(figure(1),fullfile(saveDir, 'chanMap.png'))
% Build output name and write out file
[~,filename,~] = fileparts(metaName);
fname = fullfile(saveDir, filename);
switch outType
    case 0      % tab delimited, chan, x, y, shank
       chanMapFile = [fname,'-siteCoords.txt'];
        fid = fopen( chanMapFile, 'w');
    ...
    case 1     % KS2 *.mat
        chanMapFile = [fname,'_kilosortChanMap.mat'];
        ...
        name = fname;
        save( chanMapFile, 'chanMap', 'chanMap0ind', 'connected', 'name', 'xcoords', 'ycoords', 'kcoords' );

    case 2  % strings to copy into JRC prm file
       chanMapFile = [fname,'_forJRCprm.txt'];
       nchan = numel(chans);
       fid = fopen( chanMapFile, 'w' );

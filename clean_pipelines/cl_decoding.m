function decodingAccuracy = cl_decoding(task_data, idx, keepVis, keepUnits, decoding_method, regularizationMethod, plot_regions, plotMe)
...
for iRegion = 1:size(plot_regions, 2)
    ...
    for iFold = 1:n_folds
        ...
        fit3 = glmnet(trainSet, train_trials, 'multinomial');
        %glmnetPrint(fit3);
        prediction_cv = glmnetPredict(fit3, testSet, [], 'class'); %extract coefficients at a single value of lambda%    
        prediction_cv_highlambda = prediction_cv(:,100);

        correct = test_trials(prediction_cv_highlambda == test_trials);
